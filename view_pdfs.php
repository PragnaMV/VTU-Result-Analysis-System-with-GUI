                    ...
                    if (!empty($pdfs)) {
                        foreach ($pdfs as $pdf) {
                            ...
                            <li class="list-group-item">
                                <span class="font-weight-bold"><?php echo $usn; ?></span>
                                <div>
    <a href="uploads/<?php echo urlencode($department); ?>/<?php echo urlencode($scheme); ?>/<?php echo urlencode($semester); ?>/<?php echo urlencode($pdf); ?>" 
       class="btn btn-sm" 
       style="background-color: #004488; color: white;" 
       target="_blank">View PDF</a>

    <a href="view_sgpa.php?usn=<?php echo urlencode($usn); ?>&department=<?php echo urlencode($department); ?>&scheme=<?php echo urlencode($scheme); ?>&semester=<?php echo urlencode($semester); ?>" 
       class="btn btn-sm" 
       style="background-color: #004488; color: white;">View SGPAs</a>
</div>
        ...
        <?php } else { 
            // Fetch SGPA data with the latest entry for each semester
            $sql = "SELECT s1.StudentName, s1.Semester, s1.SGPA 
                    FROM sgpa_data s1
                    WHERE s1.Usn = ? 
                    AND s1.Semester <= ? 
                    AND s1.id = (SELECT MAX(s2.id) 
                                FROM sgpa_data s2 
                                WHERE s2.Usn = s1.Usn 
        ...
        ?>
            <div class="header-main">Department: <?php echo htmlspecialchars($department); ?></div>
            <div class="header-sub">Scheme: <?php echo htmlspecialchars($scheme); ?> | Semester: <?php echo htmlspecialchars($semester); ?></div>
            <h4 class="text-center">Student Name: <strong><?php echo htmlspecialchars($student_name); ?></strong></h4>
            <h5 class="text-center">USN: <strong><?php echo htmlspecialchars($usn); ?></strong></h5>
